...
var APM = require('../assets/js/libs/udlImplementations/ArduCopter.js').APM;
...
var Platform = Backbone.Model.extend({
    ...
    initialize: function() {
        ...
        this.on('change:custom_mode', function() {
            mode = this.get('custom_mode');
            switch (mode) {
                case APM.custom_modes.STABILIZE:
                    message = 'Drone is in stabilize mode.';
                    break;
                case APM.custom_modes.ALT_HOLD:
                    message = 'Drone is in altitude hold mode.';
                    break;
                case APM.custom_modes.AUTO:
                    message = 'Drone is controlled by Routine.ly&hellip;';
                    break;
                case APM.custom_modes.GUIDED:
                    message = 'Going to location&hellip;';
                    break;
                case APM.custom_modes.LOITER:
                    message = 'Hovering until further notice.';
                    break;
                case APM.custom_modes.RTL:
                    message = 'Going to home position and landing&hellip;';
                    break;
                case APM.custom_modes.LAND:
                    message = 'Drone is landing in place, please watch closely, failsafe has probably engaged';
                    break;
                default:
                    message = 'Switching to custom_mode ' + mode;
            }
